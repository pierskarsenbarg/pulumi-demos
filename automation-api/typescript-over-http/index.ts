...
const createPulumiProgram = (content: string) => async () => {
    ...
    const bucketOwnership = new s3.BucketOwnershipControls("bucketOwnership", {
    ...
    const bucketPublicAccessBlock = new s3.BucketPublicAccessBlock("bucketpublicaccessblock", {
        bucket: siteBucket.bucket,
    ...
    }, {dependsOn: [bucketOwnership, bucketPublicAccessBlock]});
...
const ensurePlugins = async () => {
    ...
    await ws.installPlugin("aws", "v5.0.0");
};
...
app.listen(8080, () => console.info("server running on :8080"));